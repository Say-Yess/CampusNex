...
const RSVPSuccessModal = ({ isOpen, onClose, event, rsvpType = 'attending' }) => {
    if (!isOpen) return null;

    const isAttending = rsvpType === 'attending';
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-2xl shadow-xl max-w-md w-full">
                ...
                <div className="p-8 text-center">
                    {/* Success Icon */}
                    <div className={`w-16 h-16 rounded-full mx-auto mb-6 flex items-center justify-center ${isAttending ? 'bg-green-100' : 'bg-orange-100'
                        }`}>
                        {isAttending ? (
                    ...
                    <h2 className="text-2xl font-bold text-gray-900 mb-4">
                        {isAttending ? 'RSVP Confirmed!' : 'Thanks for Letting Us Know'}
                    </h2>
                    ...
                    <div className={`border rounded-lg p-4 mb-6 ${isAttending
                        ...
                        {event && (
                            <div className="mt-3 pt-3 border-t border-opacity-20 border-current">
                                <p className={`text-sm ${isAttending ? 'text-green-700' : 'text-orange-700'
                                    }`}>
